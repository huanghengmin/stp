...
public class SecurityConfigAction extends ActionSupport {
	...
	public String readIps() throws Exception {
        ...
        try {
            json = read(path);
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "安全配置","用户获取管理服务、集控采集数据接口设定IP地址成功 ");
        } catch (Exception e) {
            logger.error("安全配置", e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(), "安全配置","用户获取管理服务、集控采集数据接口设定IP地址失败 ");
        }
	...
	public String select() throws Exception {
        ...
        try {
            ...
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "安全配置","用户获取管理客户机地址成功 ");
        } catch (Exception e) {
            logger.error("安全配置", e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(), "安全配置","用户获取管理客户机地址失败 ");
        }
        actionBase.actionEnd(response, json, result);
        return null;
	}
	...
	public String delete() throws Exception {
        ...
        try {
            ...
            msg = delete(path, ip);
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "安全配置","用户删除管理客户机地址成功 ");
        } catch (Exception e) {
            logger.error("安全配置", e);
            logger.error( "安全配置,用户删除管理客户机地址失败 ");
            msg = "删除管理客户机地址失败";
        }
	...
	public String insert() throws Exception {
        ...
        try {
            String[] ip = ServletRequestUtils.getStringParameters(request, "ipArray");
            msg = insert(path, ip);
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "安全配置","用户新增管理客户机地址成功 ");
        } catch (Exception e) {
            logger.error("安全配置", e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(), "安全配置","用户新增管理客户机地址失败 ");
            msg = "新增管理客户机地址失败";
        }
        json = "{success:true,msg:'"+msg+"'}";
        actionBase.actionEnd(response, json, result);
	...
	public String update8443() throws Exception {
        ...
        try {
            ...
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "安全配置","用户更新管理服务接口设定IP地址成功 ");
        } catch (Exception e) {
            logger.error("安全配置", e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(),"安全配置","用户更新管理服务接口设定IP地址失败 ");
	...
	public String update8000() throws Exception {
        ...
        try {
            ...
            logService.newLog("INFO", SessionUtils.getAccount(request).getUserName(), "安全配置","用户更新集控采集数据接口设定IP地址成功 ");
        } catch (Exception e) {
            logger.error("安全配置", e);
            logService.newLog("ERROR", SessionUtils.getAccount(request).getUserName(),"安全配置","用户更新集控采集数据接口设定IP地址失败 ");
