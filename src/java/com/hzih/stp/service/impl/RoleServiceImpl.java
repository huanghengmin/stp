...
import com.hzih.stp.dao.RoleDao;
...
import com.hzih.stp.utils.LicenseUtils;
...
public class RoleServiceImpl implements RoleService {
    ...
    public String select(int start, int limit) throws Exception{
        ...
        for (Role role : roles) {
             if(index == start && count < limit){
                 RoleType roleType = roleTypeDao.findByCode(role.getRoleType());
                json +="{id:'"+role.getId()+"',name:'"+role.getName()+"',roleType:'"+role.getRoleType()+
                        "',description:'"+role.getDescription()+"',roleTypeName:'"+ roleType.getName() +
    ...
    @Override
    public String getTypeKeyValue() throws Exception {
        String json = "{success:true,total:";
        List<RoleType> roleTypes = roleTypeDao.findAll();
        int total = roleTypes.size();
        json += total + ",rows:[";
        for (RoleType roleType : roleTypes) {
            json +="{value:'"+roleType.getCode()+"',key:'"+roleType.getName()+"'},";
        }
        json += "]}";
        return json;
    }
