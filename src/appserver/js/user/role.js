...
Ext.onReady(function() {
    ...
    var record = new Ext.data.Record.create([
        ...
        {name:'roleTypeName',		mapping:'roleTypeName'},
...
//        boxM,
        rowNumber,
        {header:"角色名",			dataIndex:"name",		      align:'center',sortable:true},
        {header:"角色类型",		dataIndex:"roleTypeName",	  align:'center'},
        {header:"描述信息",		dataIndex:"description",	  align:'center'},
        {header:'创建时间',		dataIndex:'createTime',	  align:'center',sortable:true},
        {header:'修改时间',		dataIndex:'modifiedTime', align:'center',sortable:true,     renderer:show_null},
        {header:'操作标记',		dataIndex:'id',			  align:'center',sortable:true,		renderer:show_flag,	width:100}
...
var recordType = new Ext.data.Record.create([{name:'value',mapping:'value'}, {name:'key',mapping:'key'}]);
var readerType = new Ext.data.JsonReader({ totalProperty:'total',root:'rows'},recordType);
var storeType = new Ext.data.Store({
    url:"../../RoleAction_readTypeKeyValue.action",
    reader:readerType
});
...
function showUpdateRole(value) {
    storeType.load();
    storeType.addListener('load',function(){
        var roleType = Ext.getCmp('role.roleType.info').getValue();
        Ext.getCmp('role.roleType.info').setValue(roleType);
    });
    var record = new Ext.data.Record.create([
        {name:'id',			mapping:'id'},
        {name:'topName',		mapping:'topName'},
            ...
            formPanel = new Ext.form.FormPanel({
                ...
                items:[{
                    ...
                    items:[{
                        ...
                        items:[{
                            id:'role.roleType.info',
                            fieldLabel:"角色类型",
                            name:'role.roleType',value:item.data.roleType,
                            mode:'local',
                            editable : false,
                            typeAhead:true,
                            forceSelection: true,
